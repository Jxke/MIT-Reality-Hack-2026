            ...
            while (tcpClient != null && tcpClient.Connected)
            {
                if (tcpStream.DataAvailable)
                {
                    ...
                    while (true)
                    {
                        ...
                        string currentMessage = accumulatedData.Substring(start + 1, end - start - 1);
                        if (!string.Equals(lastMessage, currentMessage))
                        {
                            lastMessage = currentMessage;
                            onMessageRecieved.Invoke(currentMessage);
                        }
                        Debug.Log($"Message received! {currentMessage}");

                        accumulatedData = accumulatedData.Substring(end + 2); // skip "E\n"
                    }
                }
                await Task.Delay(50);
            }
